...
def main(argv):
	...
	try:
		...
		print 'OPTERROR: zenoss_get_current_avail.py --outputpath </var/www/datasources/zenoss/>'
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print 'HELP: zenoss_get_current_avail.py --outputpath </var/www/datasources/zenoss/>'
			sys.exit()
		elif opt in ("-s", "--start"):
			parastartdate = arg
		elif opt in ("-e", "--end"):
	...
	#default date for query is the whole yestrday (current day -1)
	today = datetime.date.today()
	yesterday = today - datetime.timedelta(days=1)
	parastartdate = yesterday.strftime('%Y-%m-%dT%H:%M:%SZ')
	...
	raw_events_current = z.get_current_events(None,None,'/Status/Ping',None)
	
	#import code
	#code.interact(local=locals())	
	
	#print json.dumps(raw_events_current, indent=4, sort_keys=True)
